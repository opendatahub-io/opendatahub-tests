...
class Deployment(NamespaceScopedResource):
    ...
    def wait_for_replicas(self, timeout: int = 300, poll_interval: int = 5) -> None:
        """Wait until all desired replicas are ready."""
        deadline = time.time() + timeout
        while time.time() < deadline:
            try:
                status = self._get_json().get("status", {})
                desired = self._get_json().get("spec", {}).get("replicas", 1)
