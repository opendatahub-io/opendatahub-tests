...
class Pod(NamespaceScopedResource):
    ...
    @property
    def status(self) -> str:
        """Return the pod's current phase (Running, Succeeded, Pending, Failed, etc.)."""
        return self.instance.status.phase or ""
    ...
    def log(
        ...
        Args:
            ...
            follow: Stream logs.
            since: Only return logs newer than a relative duration (e.g., 5s, 2m, 3h).
            since_time: Only return logs after a specific date (RFC3339, e.g., 2026-07-24T00:00:00Z).
    ...
    def execute(self, command: list[str], container: str | None = None, ignore_rc: bool = False) -> str:
        ...
        args.append("--")
        args.extend(command)
        return run_oc(*args, check=not ignore_rc).stdout
