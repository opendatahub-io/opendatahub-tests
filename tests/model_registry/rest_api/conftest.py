...
def registered_model_rest_api(
    request: pytest.FixtureRequest, model_registry_rest_url: list[str], model_registry_rest_headers: dict[str, str]
) -> dict[str, Any]:
...
def mysql_template_with_ca(model_registry_db_secret: list[Secret]) -> dict[str, Any]:
    ...
    mysql_template = get_model_registry_deployment_template_dict(
        ...
        resource_name=DB_BASE_RESOURCES_NAME,
    )
...
def deploy_secure_mysql_and_mr(
    ...
    mysql = get_mysql_config(deployment_name=model_registry_db_deployment[0].name, namespace=model_registry_namespace)
...
def patch_mysql_deployment_with_ssl_ca(
    ...
    mr_deployment = model_registry_db_deployment[0]
    deployment = mr_deployment.instance.to_dict()
    ...
    with ResourceEditor(patches={mr_deployment: patch}):
        mr_deployment.wait_for_condition(condition="Available", status="True")
        yield mr_deployment
