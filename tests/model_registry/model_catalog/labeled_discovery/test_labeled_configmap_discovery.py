...
class TestLabeledConfigMapDiscovery:
    ...
    def test_labeled_configmap_data_accessible_via_api(
        ...
        source_ids = [s["id"] for s in sources["items"]]
        ...
        model_names = [m["name"] for m in models_response["items"]]
    ...
    def test_labeled_configmap_model_fields_correct(
        ...
        model = execute_get_command(
            url=f"{model_catalog_rest_url[0]}sources/{TEST_SOURCE_ALPHA_ID}/models/{TEST_MODEL_ALPHA_NAME}",
            headers=model_registry_rest_headers,
        )
    ...
    def test_labeled_configmap_deployment_args_updated(
        ...
        labeled_args = [a for a in args if LABELED_SOURCES_PATH_PREFIX in a]
        assert labeled_args, f"No labeled source args found in deployment: {args}"
        assert any("test-labeled-alpha" in a for a in labeled_args), (
...
class TestMultipleLabeledConfigMaps:
    ...
    def test_multiple_labeled_configmaps_alphabetical_order(
        ...
        """
        args = get_deployment_catalog_args(admin_client=admin_client, namespace=model_registry_namespace)
        catalogs_args = [a for a in args if "--catalogs-path=" in a]

        default_indices = [i for i, a in enumerate(catalogs_args) if LABELED_SOURCES_PATH_PREFIX not in a]
        labeled_indices = [i for i, a in enumerate(catalogs_args) if LABELED_SOURCES_PATH_PREFIX in a]
        ...
        labeled_args = [catalogs_args[i] for i in labeled_indices]
        alpha_idx = next(i for i, a in enumerate(labeled_args) if "test-labeled-alpha" in a)
        beta_idx = next(i for i, a in enumerate(labeled_args) if "test-labeled-beta" in a)
    ...
    def test_both_labeled_configmaps_serve_data_via_api(
        ...
        assert any(m["name"] == TEST_MODEL_ALPHA_NAME for m in alpha_models["items"])
        ...
        assert any(m["name"] == TEST_MODEL_BETA_NAME for m in beta_models["items"])
...
class TestLabeledConfigMapDeletion:
    ...
    def test_labeled_configmap_removal_updates_deployment_and_api(
        ...
        sources = execute_get_command(url=f"{model_catalog_rest_url[0]}sources", headers=model_registry_rest_headers)
        assert deletion_source_id in [s["id"] for s in sources["items"]]
        ...
        assert deletion_source_id not in [s["id"] for s in sources_after["items"]], (
