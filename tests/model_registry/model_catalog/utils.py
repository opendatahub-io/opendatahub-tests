...
def assert_accessible_models_via_catalog_api(
    ...
    # Verify that expected accessible models are available through catalog API
    found_models = [model for model in expected_accessible_models if model in available_model_names]
    ...
    assert len(found_models) == len(expected_accessible_models), (
        f"Expected all {len(expected_accessible_models)} accessible models to be found, "
        f"but only found {len(found_models)}. "
        f"Expected: {expected_accessible_models}, "
