...
def get_mr_service_objects(
    base_name: str, namespace: str, client: DynamicClient, teardown_resources: bool, num: int = 1
) -> list[Service]:
    services = []
    annotation = {
    ...
    for num_deployment in range(0, num):
        ...
        deployments.append(
            Deployment(
                ...
                template=get_model_registry_deployment_template_dict(secret_name=secret.name, resource_name=name),
...
def get_model_registry_objects(
    ...
    for num_mr in range(0, num):
        ...
        deployment = deployments[num_mr]

        mysql = get_mysql_config(deployment_name=deployment.name, namespace=namespace)
...
def get_mysql_config(deployment_name: str, namespace: str) -> dict[str, Any]:
    return {
        "host": f"{deployment_name}.{namespace}.svc.cluster.local",
        "database": MODEL_REGISTRY_DB_SECRET_STR_DATA["database-name"],
        "passwordSecret": {"key": "database-password", "name": deployment_name},
        "port": 3306,
