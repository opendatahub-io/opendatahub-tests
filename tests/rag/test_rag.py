...
from utilities.rag_utils import validate_rag_agent_responses
...
class TestRag:
    ...
    def test_rag_deployment(
        ...
        """
        Test LlamaStack deployment and verify required models are available.
        ...
        assert model_id is not None, "No llm model returned from LlamaStackClient"

        embedding_model_id = (em := next(m for m in models if m.api_model_type == "embedding")).identifier
    ...
    def test_rag_simple_agent(self, lls_client: LlamaStackClient) -> None:
        ...
        response = agent.create_turn(
            messages=[{"role": "user", "content": "Who are you?"}],  # type: ignore
            session_id=s_id,
            stream=False,
        )
        content = response.output_message.content  # type: ignore
        assert content is not None, "LLM response content is None"
        assert "model" in content, "The LLM didn't provide the expected answer to the prompt"  # type: ignore

        # Test capability question
        response = agent.create_turn(
            messages=[{"role": "user", "content": "What can you do?"}],  # type: ignore
            session_id=s_id,
            stream=False,
        )
        content = response.output_message.content  # type: ignore
        assert content is not None, "LLM response content is None"
        assert "answers" in content, "The LLM didn't provide the expected answer to the prompt"  # type: ignore
