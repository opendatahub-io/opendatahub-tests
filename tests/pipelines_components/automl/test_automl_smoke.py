...
class TestAutoMLSmoke:
    ...
    def test_automl_pipeline_completes(
        ...
        Pipeline source: red-hat-data-services/pipelines-components
          pipelines/training/automl/autogluon_tabular_training_pipeline/pipeline.yaml

        The test uploads a compiled pipeline YAML to the DSPA, creates a run, polls the
        Argo Workflow status until completion, and asserts that the workflow phase is Succeeded.
        """
