...
def autorag_postgres_deployment(
    ...
    with Deployment(
        ...
        template=get_postgres_template(secret_name=autorag_ogx_secret.name, app_label=app_label),
    ) as deployment:
...
def autorag_etcd_deployment(
    ...
    template = get_etcd_template(etcd_service_name=service_name)
...
def autorag_ogx_deployment(
    ...
    wait_for_unique_ogx_pod(client=admin_client, namespace=autorag_ogx_server.namespace)
...
def autorag_discovered_models(
    ...
    registered_ids = log_registered_models(client=autorag_ogx_client)
