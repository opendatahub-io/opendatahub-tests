...
def evalhub_preflight_verified(
    ...
    """Run EvalHub API + Kueue checks before any test under this package.

    Calls ``verify_evalhub_preflight`` (token, health probe, 404 on fake job id,
    Kueue CRDs/controller). For a standalone run, use ``test_evalhub_preflight.py`` in this directory.
    """
    try:
