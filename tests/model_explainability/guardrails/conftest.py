...
def tempo_stack(
    ...
    wait_for_pods_by_label(client=admin_client, namespace=model_namespace.name, label_selector=label_selector)
...
def otel_collector(
    ...
    otel_cr_dict["spec"]["config"] = {
        "exporters": {
            "otlp": {
                "endpoint": (
                    f"tempo-{tempo_stack.metadata.name}-distributor.{namespace}.svc.cluster.local:{OTEL_EXPORTER_PORT}"
                ),
                "tls": {"insecure": True},
            }
        },
...
def minio_secret_otel(admin_client, model_namespace, minio_service_otel):
    secret = Secret(
        ...
        string_data={
            ...
            "access_key_secret": "supersecret",  # pragma: allowlist secret
        },
        type="Opaque",
        teardown=True,
    )
