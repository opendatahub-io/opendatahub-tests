"""TC-NEG-001: vLLM deployment with corrupted/missing model in S3.

Validates that the ISVC surfaces a descriptive error when the S3 storage URI
points to a non-existent or corrupted model path.
"""
...
class TestCorruptedModelDeployment:
    def test_isvc_fails_with_storage_error(
        ...
        """Given a vLLM ISVC pointing to a non-existent S3 model path,
