...
@pytest.mark.parametrize(
    ...
    [
        ...
        pytest.param(
            {"name": "llama-instruct-8b-serverless"},
            {"model-dir": MODEL_PATH},
            {"deployment_type": KServeDeploymentType.SERVERLESS},
            {
                **BASE_SEVERRLESS_DEPLOYMENT_CONFIG,
                "gpu_count": 1,
                "name": "llama-instruct-ser",
            },
            id="llama-instruct-8b-serverless-single-gpu",
        ),
    ],
    indirect=True,
)
class TestLlamaInstructModel:
    def test_llama3_instruct_8b_raw_simple_openai_model_inference(
        ...
        skip_if_serverless_deployment: Any,
...
@pytest.mark.parametrize(
    ...
    [
        pytest.param(
            {"name": "llama-instruct-8b-raw-multi"},
            {"model-dir": MODEL_PATH},
            {"deployment_type": KServeDeploymentType.RAW_DEPLOYMENT},
            {
                **BASE_RAW_DEPLOYMENT_CONFIG,
                "gpu_count": 2,
                "name": "llama-instruct-rm",
            },
            id="llama-instruct-8b-raw-multi-gpu",
        ),
        pytest.param(
            {"name": "llama-instruct-8b-serverless-multi"},
            {"model-dir": MODEL_PATH},
            {"deployment_type": KServeDeploymentType.SERVERLESS},
            {
                **BASE_SEVERRLESS_DEPLOYMENT_CONFIG,
                "gpu_count": 2,
                "name": "llama-instruct-sm",
            },
            id="llama-instruct-8b-serverless-multi-gpu",
        ),
    ],
    indirect=True,
)
class TestMultiLlamaInstructModel:
