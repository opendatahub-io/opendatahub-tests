...
def build_pytest_markers(deployment_type: str, execution_mode: str) -> List[Any]:
    ...
    # Base deployment markers
    base_marker_map = {
        KServeDeploymentType.RAW_DEPLOYMENT: pytest.mark.rawdeployment,
        KServeDeploymentType.SERVERLESS: pytest.mark.serverless,
    }

    if deployment_type not in base_marker_map:
        return markers

    markers.append(base_marker_map[deployment_type])

    # Execution mode markers
    if execution_mode == "parallel":
        markers.append(pytest.mark.parallel)
    else:
        markers.append(pytest.mark.sequential)
...
def pytest_generate_tests(metafunc: pytest.Metafunc) -> None:
    ...
    for model_car in model_car_data:
        ...
        execution_mode = model_car.get("execution_mode", "sequential").strip()
