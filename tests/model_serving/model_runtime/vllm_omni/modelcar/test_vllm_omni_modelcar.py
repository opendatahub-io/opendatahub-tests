...
pytestmark = [
    ...
    pytest.mark.usefixtures("skip_if_no_supported_accelerator_type"),
]
...
class TestVllmOmniModelcar:
    def test_vllm_omni_modelcar_qwen3_tts_inference(
        ...
        assert response.status_code == 200, (
            f"/v1/audio/speech returned HTTP {response.status_code}. Body (first 500 chars): {response.text[:500]}"
        )

        content_type = response.headers.get("Content-Type", "").split(";")[0].strip()
        assert content_type in VALID_AUDIO_CONTENT_TYPES, (
            f"Expected Content-Type in {VALID_AUDIO_CONTENT_TYPES}, got '{content_type}'"
        )

        assert len(response.content) > 44, (
            f"Response body is {len(response.content)} bytes — too small; "
            f"expected more than 44 bytes (WAV header minimum). "
            "The model may have returned an empty or truncated audio payload."
        )
        assert response.content[:4] == b"RIFF", f"Expected WAV RIFF magic, got {response.content[:4]!r}"
        ...
        predictor_pods = get_pods_by_isvc_label(
            client=admin_client,
            isvc=vllm_omni_modelcar_inference_service,
        )
        assert predictor_pods, (
            f"No predictor pods found for ISVC '{vllm_omni_modelcar_inference_service.name}' "
            f"in namespace '{model_namespace.name}'. "
            "Verify the InferenceService reached Ready state and the modelcar image was pulled successfully."
        )
        for predictor_pod in predictor_pods:
            for container_status in predictor_pod.instance.status.containerStatuses or []:
                restart_count: int = container_status.restartCount
                assert restart_count == 0, (
                    f"Container '{container_status.name}' in pod "
                    f"'{predictor_pod.name}' restarted {restart_count} time(s)."
                )
