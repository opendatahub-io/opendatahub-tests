...
conftest until fast templates are available on the cluster.
"""
...
pytestmark = [
    ...
    pytest.mark.usefixtures("skip_if_no_vllm_omni_multi_gpu", "valid_aws_config"),
]
...
class TestVllmOmniFast1Inference:
    """Validate vLLM-Omni inference using the fast-1 ServingRuntime template."""

    def test_fast_1_health(self, vllm_omni_inference_service: InferenceService, vllm_omni_isvc_url: str) -> None:
        """Given a vLLM-Omni ISVC deployed with the fast-1 runtime template,
        When a GET /health request is sent,
        Then the response is HTTP 200.
        """
        url = vllm_omni_isvc_url
        response = requests.get(f"{url}{HEALTH_ENDPOINT}", verify=False, timeout=30)
        assert response.status_code == 200, f"GET {HEALTH_ENDPOINT} returned HTTP {response.status_code}; expected 200"

    def test_fast_1_tts_inference(
        self,
    ...
    def test_fast_1_vllm_omni_version(
        ...
        url = vllm_omni_isvc_url
        response = requests.get(f"{url}{VERSION_ENDPOINT}", verify=False, timeout=30)
...
class TestVllmOmniFast2Inference:
    """Validate vLLM-Omni inference using the fast-2 ServingRuntime template."""

    def test_fast_2_health(self, vllm_omni_inference_service: InferenceService, vllm_omni_isvc_url: str) -> None:
        """Given a vLLM-Omni ISVC deployed with the fast-2 runtime template,
        When a GET /health request is sent,
        Then the response is HTTP 200.
        """
        url = vllm_omni_isvc_url
    ...
    def test_fast_2_vllm_omni_version(
        self, vllm_omni_inference_service: InferenceService, vllm_omni_isvc_url: str
    ) -> None:
        """Verify the fast template serves a valid vLLM-Omni version."""
        url = vllm_omni_isvc_url
