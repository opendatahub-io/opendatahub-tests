...
class TestVllmOmniFullE2E:
    ...
    def test_vllm_omni_health_endpoint(
        ...
        response = requests.get(f"{base_url}{HEALTH_ENDPOINT}", verify=False, timeout=30)
        assert response.status_code == HTTPStatus.OK, (
            f"GET {HEALTH_ENDPOINT} returned HTTP {response.status_code}; "
    ...
    def test_vllm_omni_50_turn_stability(
        ...
        assert not restarted, (
            f"50-turn chat stability failed: containers restarted during session: {restarted}. "
