...
from tests.model_serving.model_runtime.vllm_omni.utils import _JPEG_MAGIC, _PNG_MAGIC
...
class TestVllmOmniFLUX2ImageGeneration:
    ...
    def test_vllm_omni_flux2_image_generation(
        ...
        final_restarts = get_restart_counts(pod=vllm_omni_pod_resource)
        restarted = {
            name: final_restarts[name] - initial_restarts.get(name, 0)
            for name in final_restarts
            if final_restarts[name] > initial_restarts.get(name, 0)
        }
        assert not restarted, (
            f"Pod '{vllm_omni_pod_resource.name}' containers restarted during FLUX.2 inference: {restarted}. "
            f"Initial: {initial_restarts}, final: {final_restarts}. "
            "Check pod logs for OOM kills or GPU errors."
        )
