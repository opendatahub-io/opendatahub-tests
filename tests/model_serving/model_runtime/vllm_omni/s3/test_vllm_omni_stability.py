...
pytestmark = [
    ...
    pytest.mark.usefixtures("skip_if_no_supported_accelerator_type", "valid_aws_config"),
]
...
class TestVllmOmniQwen3TtsStabilityAndPerformance:
    ...
    def test_vllm_omni_tts_50_turn_session(
        ...
        for warm_idx in range(WARM_UP_COUNT):
            status_code, _, _ = timed_tts_request(
                url=base_url,
                model=vllm_omni_inference_service.instance.metadata.name,
                prompt=TTS_PROMPT_CORPUS[warm_idx % len(TTS_PROMPT_CORPUS)],
                voice=OMNI_TTS_VOICE[QWEN3_TTS_MODEL_PATH],
            )
            LOGGER.info(event="warm-up request", index=warm_idx, status_code=status_code)
        ...
        final_restart_counts = get_restart_counts(pod=vllm_omni_pod_resource)
        restarted = [
            name for name, count in final_restart_counts.items() if count != initial_restart_counts.get(name, 0)
        ]
        assert not restarted, (
            f"50-turn TTS stability failed: containers restarted during session: {restarted}. "
            f"Before: {initial_restart_counts}, after: {final_restart_counts}"
        )
    ...
    def test_vllm_omni_tts_ttfb_and_e2e_latency(
        ...
        Then TTFB p95 is at most 350 ms and E2E p95 is at most 1.2 s, both measured at
        the OpenShift Route, providing at least 200 data points for statistical validity.
        """
        base_url = vllm_omni_isvc_url
        LOGGER.info(
...
class TestVllmOmniSoakComposite:
    """Composite soak test — tier3 only (10-min continuous dialogue)."""
    ...
    def test_vllm_omni_composite_soak(
        ...
        assert not restarted, (
            f"Composite soak failed: containers restarted during soak: {restarted}. "
            f"Before: {initial_restart_counts}, after: {final_restart_counts}"
        )
        ...
        growth_ratio = (final_gpu_usage - baseline_gpu_usage) / baseline_gpu_usage if baseline_gpu_usage > 0 else 0.0
        LOGGER.info(
...
class TestVllmOmniVoiceModelStability:
    ...
    def test_vllm_omni_voice_50_turn_stability(
        ...
        voice = OMNI_TTS_VOICE.get(model_dir)
        initial_restart_counts = get_restart_counts(pod=vllm_omni_pod_resource)
        LOGGER.info(
        ...
        assert not restarted, (
            f"50-turn stability FAILED for {model_name!r}: containers restarted: {restarted}. "
            f"Before: {initial_restart_counts}, after: {final_restart_counts}"
        )
