...
class TestVllmOmniDSProjectUserRBAC:
    ...
    def test_vllm_omni_ds_project_user_cannot_modify_cluster_runtime(
        ...
        """Verify DS Project User receives 403 when patching a cluster-level ServingRuntime.
        ...
        assert exc_info.value.status == 403, (
            f"Expected HTTP 403 Forbidden for DS Project User patching cluster ServingRuntime, "
            f"got HTTP {exc_info.value.status}. "
            "DS Project Users must not modify shared cluster-level runtime templates."
        )
        LOGGER.info(
            event="DS Project User correctly denied PATCH on cluster ServingRuntime",
