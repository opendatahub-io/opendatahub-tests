...
pytestmark = [pytest.mark.vllm_omni, pytest.mark.downstream_only, pytest.mark.skip_must_gather]
...
class TestVllmOmniTemplateStructure:
    ...
    def test_vllm_omni_runtime_template_exists(
        ...
        assert container_image in csv_images, (
            f"Template image '{container_image}' is not present in the operator CSV relatedImages. "
            f"The template must use an image tracked by the operator for supply-chain integrity. "
            f"CSV contains {len(csv_images)} relatedImages."
        )
        LOGGER.info(event="vLLM-Omni template exists with CSV-tracked sha256 digest image", image=container_image)
