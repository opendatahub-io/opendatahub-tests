...
from tests.model_serving.model_runtime.vllm_omni.utils import _WAV_HEADER_MIN_BYTES
...
class TestVllmOmniPvcQwen3TtsInference:
    ...
    def test_vllm_omni_pvc_storage_inference(
        ...
        assert response.status_code == 200, (
            f"Expected HTTP 200 from /v1/audio/speech, got {response.status_code}. Body: {response.text[:200]}"
        )
        content_type: str = response.headers.get("Content-Type", "")
        assert content_type in {"audio/wav", "audio/x-wav"}, (
            f"Expected Content-Type 'audio/wav' or 'audio/x-wav', got: {content_type!r}"
        )
        response_size: int = len(response.content)
        assert response_size > _WAV_HEADER_MIN_BYTES, (
            f"Response body ({response_size} bytes) is too small; "
            f"expected > {_WAV_HEADER_MIN_BYTES} bytes (WAV header minimum)"
        )
