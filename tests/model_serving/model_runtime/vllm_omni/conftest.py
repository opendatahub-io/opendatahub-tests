...
def vllm_omni_inference_service(
    ...
    model_path = request.param.get("model_path")
    if not model_path:
        model_dir = s3_models_storage_uri.rstrip("/").rsplit("/", 1)[-1]
        model_path = model_dir if model_dir in OMNI_ISVC_TIMEOUTS else QWEN3_OMNI_MODEL_PATH
    timeout = request.param.get("timeout", OMNI_ISVC_TIMEOUTS.get(model_path, 1800))
