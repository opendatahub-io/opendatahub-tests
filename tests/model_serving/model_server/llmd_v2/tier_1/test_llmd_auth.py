...
class TestLLMISVCAuth:
    ...
    def test_llmisvc_unauthorized(self, llmisvc_with_auth):
        """Test that unauthorized access to LLMInferenceServices is properly blocked."""
        svc_a = llmisvc_with_auth[0]["service"]
        token_b = llmisvc_with_auth[1]["token"]

        # User B's token cannot access user A's service
        status, _ = send_chat_completions(svc_a, prompt="What is the capital of Italy?", token=token_b, insecure=False)
        assert status in (401, 403), f"Cross-user access should be denied, got {status}"

        # No token at all fails
        status, _ = send_chat_completions(svc_a, prompt="What is the capital of Italy?", insecure=False)
