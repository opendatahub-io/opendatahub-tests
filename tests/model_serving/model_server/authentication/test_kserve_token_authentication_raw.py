...
class TestKserveTokenAuthenticationRawForRest:
    ...
    def test_model_authentication_using_rest_raw(self, http_ovms_raw_inference_service, http_ovms_raw_inference_token):
        """Verify RAW Kserve model query with token using REST"""
        verify_inference_response(
            inference_service=http_ovms_raw_inference_service,
            inference_config=ONNX_INFERENCE_CONFIG,
            inference_type=Inference.INFER,
            protocol=Protocols.HTTPS,
            use_default_query=True,
            token=http_ovms_raw_inference_token,
        )

    @pytest.mark.dependency(name="test_disabled_raw_model_authentication")
    def test_disabled_raw_model_authentication(self, patched_remove_ovms_raw_authentication_isvc):
        """Verify model query after authentication is disabled"""
        verify_inference_response(
            inference_service=patched_remove_ovms_raw_authentication_isvc,
            inference_config=ONNX_INFERENCE_CONFIG,
            inference_type=Inference.INFER,
            protocol=Protocols.HTTPS,
            use_default_query=True,
    ...
    def test_raw_disable_enable_authentication_no_pod_rollout(self, http_ovms_raw_inference_service):
        """Verify no pod rollout when disabling and enabling authentication"""
        pod = get_pods_by_isvc_label(
            client=http_ovms_raw_inference_service.client,
    ...
    def test_re_enabled_raw_model_authentication(self, http_ovms_raw_inference_service, http_ovms_raw_inference_token):
    ...
    def test_cross_model_authentication_raw(
        self, http_ovms_raw_inference_service_2, http_ovms_raw_inference_token, admin_client
    ):
        ...
        if is_jira_open(jira_id="RHOAIENG-19645", admin_client=admin_client):
            inference = UserInference(
                inference_service=http_ovms_raw_inference_service_2,
                inference_config=ONNX_INFERENCE_CONFIG,
                inference_type=Inference.INFER,
                protocol=Protocols.HTTPS,
            )
            ...
            res = inference.run_inference_flow(
                use_default_query=True, token=http_ovms_raw_inference_token, insecure=False
            )
