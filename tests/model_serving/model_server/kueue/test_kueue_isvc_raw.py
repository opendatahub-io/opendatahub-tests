...
min_replicas = 1 # min_replicas needs to be 1 or you need to change the test to check for the number of available replicas
max_replicas = 2

@pytest.mark.rawdeployment
@pytest.mark.parametrize(
    "unprivileged_model_namespace, kueue_kserve_serving_runtime, kueue_raw_inference_service, kueue_cluster_queue_from_template, kueue_resource_flavor_from_template, kueue_local_queue_from_template",
    [
        pytest.param(
            {"name": "kueue-isvc-raw-test", "add-kueue-label": True},
            RunTimeConfigs.ONNX_OPSET13_RUNTIME_CONFIG,
            {"name": "kueue-isvc-raw", "min-replicas": min_replicas, "max-replicas": max_replicas, "labels": {"kueue.x-k8s.io/queue-name": local_queue_name}, "deployment-mode": KServeDeploymentType.RAW_DEPLOYMENT, "model-dir": "test-dir", "model-version": ModelVersion.OPSET13, "resources": isvc_resources},
            {"name": cluster_queue_name, "resource_flavor_name": resource_flavor_name, "cpu_quota": cpu_quota, "memory_quota": memory_quota, "namespace_selector": {}},
...
class TestKueueInferenceServiceRaw:
    def test_kueue_inference_service_raw(self, admin_client, kueue_cluster_queue_from_template, kueue_resource_flavor_from_template, kueue_local_queue_from_template, kueue_raw_inference_service, kueue_kserve_serving_runtime):
        """Test inference service with model mesh deployment"""
