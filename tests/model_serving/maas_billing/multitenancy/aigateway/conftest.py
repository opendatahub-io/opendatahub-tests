...
def aigateway_infra_namespace(admin_client: DynamicClient) -> str:
    ...
    assert infra_namespace.exists, (
        f"Infra namespace '{AIGATEWAY_INFRA_NAMESPACE}' not found — required for AITenant multitenancy tests"
    )
...
def aigateway_for_test(
    ...
    """Create a disposable AITenant and yield context for bootstrap assertions."""
    aigateway_name = f"e2e-aigw-{generate_random_name()}"
    aigateway_spec = build_aigateway_spec(aigateway_name=aigateway_name)
    with ready_aigateway_with_preprovisioned_gateway(
...
def aigateway_with_oidc(
    ...
    """Create an AITenant with spec.oidc configured."""
...
@pytest.fixture
def ready_aigateway_for_deletion(
    admin_client: DynamicClient,
    aigateway_infra_namespace: str,
    teardown_resources: bool,
) -> Generator[AIGatewayTestContext, Any, Any]:
    """Create a Ready AITenant for deletion tests; test owns deletion."""
...
def aigateway_pending_missing_tenant_namespace(
    ...
    """Create an AITenant with create=false against a missing tenant namespace."""
...
def invalid_placement_aigateway(
    ...
    """Create an AITenant in an invalid namespace placement scenario."""
