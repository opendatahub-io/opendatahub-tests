...
UPGRADE_DISTRIBUTION_NAME = os.getenv("LLS_UPGRADE_DISTRIBUTION_NAME", "llama-stack-distribution-upgrade")
UPGRADE_UNPRIVILEGED_DISTRIBUTION_NAME = os.getenv(
    "LLS_UPGRADE_UNPRIVILEGED_DISTRIBUTION_NAME", "llama-stack-distribution-unprivileged-upgrade"
)
DEFAULT_DISTRIBUTION_NAME = generate_random_name(prefix="llama-stack-distribution")
DEFAULT_UNPRIVILEGED_DISTRIBUTION_NAME = generate_random_name(prefix="llama-stack-distribution")


def _get_distribution_name(pytestconfig: pytest.Config) -> str:
    if pytestconfig.option.pre_upgrade or pytestconfig.option.post_upgrade:
        return UPGRADE_DISTRIBUTION_NAME
    return DEFAULT_DISTRIBUTION_NAME


def _get_unprivileged_distribution_name(pytestconfig: pytest.Config) -> str:
    if pytestconfig.option.pre_upgrade or pytestconfig.option.post_upgrade:
        return UPGRADE_UNPRIVILEGED_DISTRIBUTION_NAME
    return DEFAULT_UNPRIVILEGED_DISTRIBUTION_NAME
...
def llama_stack_server_config(
    ...
    if params.get("enable_ragas_remote"):
        ...
        distribution_name = _get_distribution_name(pytestconfig=pytestconfig)
...
def unprivileged_llama_stack_distribution(
    ...
    distribution_name = _get_unprivileged_distribution_name(pytestconfig=pytestconfig)
    if pytestconfig.option.post_upgrade:
