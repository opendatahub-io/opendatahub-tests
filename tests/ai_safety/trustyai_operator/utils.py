...
def validate_trustyai_operator_image(
    ...
    Checks if:
        ...
        - image complies with OpenShift AI requirements i.e. sourced from the registry matching the installed
          operator channel (registry.redhat.io for released channels, quay.io for odh-stable) and pinned w/o tags.
        ...
        Args:
            ...
            expected_image_registry (str): registry the operator image is expected to be served from
    ...
    image_valid, error_message = validate_image_format(
        image=tai_operator_image, expected_registry=expected_image_registry
    )
