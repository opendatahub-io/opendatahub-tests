...
**Location:** `tests/ai_safety/lm_eval/upgrade/test_lm_eval_upgrade.py`

**Test Classes:**
- `TestLMEvalJobLifecyclePreUpgrade`
- `TestLMEvalJobLifecyclePostUpgrade`

**Covered Upgrade Paths:**
- LMEvalJob lifecycle across platform upgrade
  - Pre-upgrade: Start an LMEvalJob with a basic evaluation task (arc_easy)
  - Post-upgrade: Verify job completes successfully
  - Validated: Job pod lifecycle, completion status

**What's Validated:**
- LMEvalJob pods started before upgrade continue running
- Jobs complete successfully after platform upgrade
- Task execution is not interrupted by the upgrade process

**Example:**
```bash
# Pre-upgrade
uv run pytest -m pre_upgrade tests/ai_safety/lm_eval/upgrade/

# Perform platform upgrade

# Post-upgrade (skipped on disconnected environments)
uv run pytest -m "post_upgrade and not skip_on_disconnected" tests/ai_safety/lm_eval/upgrade/
```
