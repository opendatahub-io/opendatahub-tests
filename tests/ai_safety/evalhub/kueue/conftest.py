...
from ocp_resources.cluster_role import ClusterRole as OcpClusterRole
...
def trustyai_pods_log_rbac(
    ...
    """Give the TrustyAI operator permission to read pod logs using a separate ClusterRole.
    ...
    permission. Editing its managed ClusterRole does not work because RHOAI
    automatically restores the original configuration. A separate ClusterRole
    and binding provide the required permission without being overwritten.
    """
    sa_name = "trustyai-service-operator-controller-manager"
    sa_namespace = "redhat-ods-applications"
    cluster_role_name = "trustyai-pods-log-grant"

    cluster_role = OcpClusterRole(
        client=admin_client,
        name=cluster_role_name,
    )
    if cluster_role.exists:
        LOGGER.info(f"ClusterRole {cluster_role_name} already exists, skipping creation")
        yield
        return

    LOGGER.info(f"Creating ClusterRole {cluster_role_name} for TrustyAI pods/log access")
    with (
        OcpClusterRole(
            client=admin_client,
            name=cluster_role_name,
        ...
        ClusterRoleBinding(
            client=admin_client,
            name=f"{cluster_role_name}-binding",
