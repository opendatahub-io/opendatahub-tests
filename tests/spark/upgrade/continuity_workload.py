...
def upgrade_task(value: int) -> int:
    """Keep one task active for up to 15 minutes while waiting for post-upgrade release.
    ...
    Raises:
        TimeoutError: If the post-upgrade test does not release the task within 15 minutes.
    """
    ...
    for _ in range(180):
        if Path("/opt/spark/upgrade/release").read_text().strip() == "true":
            return value * value
        time.sleep(5)
    raise TimeoutError("Post-upgrade release was not received within 15 minutes")
