...
@pytest.mark.parametrize(
    ...
    "orchestrator_config, guardrails_orchestrator, ogx_server_config",
    [
        pytest.param(
            {"name": "test-ogx-gorch"},
...
class TestOgxFMSGuardrailsProvider:
    """
    Adds basic tests for the OGX FMS Guardrails provider.

    Given a basic guardrails setup (generator model + detectors),
    and a ogx distribution and client:
    ...
    def test_fms_guardrails_register_shield(
        ...
        patched_ogx_deployment_tls_certs,
        ogx_client,
    ):
        shield_params = {
            "type": "content",
        ...
        ogx_client.shields.register(
            shield_id=SECURE_SHIELD_ID,
            provider_shield_id=SECURE_SHIELD_ID,
            provider_id=OgxProviders.Safety.TRUSTYAI_FMS,
            params=shield_params,
            timeout=120,
        )
        shields = ogx_client.shields.list()

        assert len(shields) == 1
        assert shields[0].identifier == SECURE_SHIELD_ID
        assert shields[0].provider_id == OgxProviders.Safety.TRUSTYAI_FMS
        assert shields[0].params == shield_params

    def test_fms_guardrails_run_shield(self, minio_pod, minio_data_connection, ogx_client):
        run_shields_response = ogx_client.safety.run_shield(
            shield_id=SECURE_SHIELD_ID,
            messages=[
                {
    ...
    def test_fms_moderations(self, minio_pod, minio_data_connection, ogx_client):
        ...
        """
        moderations_response = ogx_client.moderations.create(
