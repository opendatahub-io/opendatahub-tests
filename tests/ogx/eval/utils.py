...
def wait_for_eval_job_completion(
    ogx_client, job_id: str, benchmark_id: str, wait_timeout: int = 600, sleep: int = 30
) -> None:
    """
    Wait for a OGX eval job to complete.

    Args:
        ogx_client: The OGX client instance
    ...
    samples = TimeoutSampler(
        ...
        sleep=sleep,
        func=lambda: ogx_client.alpha.eval.jobs.status(job_id=job_id, benchmark_id=benchmark_id).status,
    )
