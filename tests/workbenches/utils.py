...
def load_default_notebook(client: DynamicClient, namespace: str, name: str, image: str) -> Notebook:
    ...
    username = get_username(client=client)
...
def get_username(client: DynamicClient) -> str:
    ...
    self_subject_review_resource: Resource = client.resources.get(
        api_version="authentication.k8s.io/v1", kind="SelfSubjectReview"
    )
    self_subject_review: ResourceInstance = client.create(self_subject_review_resource)
