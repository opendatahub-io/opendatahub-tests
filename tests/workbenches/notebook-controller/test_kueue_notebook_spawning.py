...
LOW_CPU_QUOTA = 0.1  # Very low CPU quota - 100m
...
class TestKueueNotebookController:
    ...
    def test_kueue_notebook_admission_control(
        ...
        print(f"Checking ClusterQueue {kueue_notebook_cluster_queue.name} resource usage...")
        cluster_queue_usage = get_queue_resource_usage(
            queue=kueue_notebook_cluster_queue, flavor_name=RESOURCE_FLAVOR_NAME
        )
        print(f"ClusterQueue usage: {cluster_queue_usage}")

        print(f"Checking LocalQueue {kueue_notebook_local_queue.name} resource usage...")
        local_queue_usage = get_queue_resource_usage(queue=kueue_notebook_local_queue, flavor_name=RESOURCE_FLAVOR_NAME)
        print(f"LocalQueue usage: {local_queue_usage}")
    ...
    def test_kueue_notebook_resource_constraints(
        ...
        assert cpu_value <= CPU_QUOTA, f"CPU request {cpu_value} should not exceed queue quota {CPU_QUOTA}"
    ...
    def test_kueue_notebook_resource_starvation(
        ...
        # Wait a bit to ensure the pod state stabilizes
    ...
    def test_kueue_notebook_stop_start_workbench(
        ...
        print(f"SUCCESS: Notebook pod {notebook_pod.name} is initially running and ready")

        # Verify Kueue is tracking exact resource usage for the running workload
        print("Checking queue resource usage before stopping workbench...")
        ...
        print(f"Initial ClusterQueue usage: {initial_cluster_usage}")
        print(f"Initial LocalQueue usage: {initial_local_usage}")
        ...
        with ResourceEditor(patches={default_notebook: stop_patch}):
            print(f"Applied kubeflow-resource-stopped annotation with timestamp: {stop_timestamp}")

            # Wait for the pod to be terminated (it should be deleted)
            notebook_pod.wait_deleted(timeout=Timeout.TIMEOUT_2MIN)

            print(f"SUCCESS: Notebook pod {notebook_pod.name} was terminated after stop annotation")

            # Verify resource usage goes to zero after stopping workbench
            print("Checking queue resource usage after stopping workbench...")
            ...
            print(f"Stopped ClusterQueue usage: {stopped_cluster_usage}")
            print(f"Stopped LocalQueue usage: {stopped_local_usage}")
        ...
        with ResourceEditor(patches={default_notebook: restart_patch}):
            print("Removed kubeflow-resource-stopped annotation to restart workbench")
            ...
            print(f"SUCCESS: Notebook pod {new_notebook_pod.name} was successfully restarted and is ready again")

            # Final verification: Ensure Kueue is tracking exact resource usage for the restarted workload
            print("Checking queue resource usage after restarting workbench...")
            ...
            print(f"Restarted ClusterQueue usage: {restarted_cluster_usage}")
            print(f"Restarted LocalQueue usage: {restarted_local_usage}")
...
def test_kueue_config_frameworks_enabled(
    ...
    print("SUCCESS: kueue-controller-manager deployment is ready and available")
