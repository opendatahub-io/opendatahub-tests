...
@pytest.mark.usefixtures("post_upgrade_mcp_config_map")
class TestPostUpgradeMCPCatalog:
    """Validates MCP catalog custom source persists after upgrade."""

    @pytest.mark.order("last")
    @pytest.mark.post_upgrade
    def test_validate_mcp_sources(
        self: Self,
        post_upgrade_mcp_config_map: ConfigMap,
    ):
        """Given a custom MCP catalog source was configured before upgrade
        When reading the ConfigMap after upgrade
        Then the custom source still exists in mcp_catalogs
        """
        sources_data = yaml.safe_load(post_upgrade_mcp_config_map.instance.data["sources.yaml"])
        mcp_catalogs = sources_data.get("mcp_catalogs", [])
        matching = [cat for cat in mcp_catalogs if cat.get("id") == MCP_CATALOG_SOURCE_ID]
        assert matching, (
            f"Custom MCP source '{MCP_CATALOG_SOURCE_ID}' not found in ConfigMap mcp_catalogs after upgrade"
        )
        LOGGER.info(f"Post-upgrade: MCP catalog source '{MCP_CATALOG_SOURCE_ID}' persisted")
