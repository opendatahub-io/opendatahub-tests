...
# Install oc (OpenShift CLI) — bootstrap only, replaced at runtime by docker-entrypoint.sh
ARG TARGETARCH
RUN case "$TARGETARCH" in \
        amd64) OC_ARCH_SUFFIX="" ;; \
        arm64) OC_ARCH_SUFFIX="-arm64" ;; \
        ppc64le) OC_ARCH_SUFFIX="-ppc64le" ;; \
        s390x) OC_ARCH_SUFFIX="-s390x" ;; \
        *) echo "Unsupported TARGETARCH: $TARGETARCH" >&2; exit 1 ;; \
    esac \
    && curl -sSL "https://mirror.openshift.com/pub/openshift-v4/clients/ocp/stable/openshift-client-linux${OC_ARCH_SUFFIX}.tar.gz" -o /tmp/oc.tar.gz \
...
RUN chmod +x $TESTS_DIR/docker-entrypoint.sh
